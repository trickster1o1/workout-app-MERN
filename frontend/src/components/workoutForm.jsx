  ...
  const handleSubmit = async (e) => {
    ...
    if (user) {
      await fetch("/api/workouts", {
        ...
        setError({reps: {kind: 'Unauthorized!!!'}});
    }
  };
  return (
    <form className="create" onSubmit={handleSubmit}>
      ...
      />
      {error.title ? (
        <span className="error">{error.title.kind.toUpperCase()}</span>
      ) : null}
      <label>Load (Kg):</label>
      <input
        type="number"
      ...
      />
      {error.load ? (
        <span className="error">{error.load.kind.toUpperCase()}</span>
      ) : null}
      <label>Reps:</label>
      <input
        type="number"
      ...
      />
      {error.reps ? (
        <span className="error">{error.reps.kind.toUpperCase()}</span>
      ) : null}
      <button>Add</button>
    </form>
  );
